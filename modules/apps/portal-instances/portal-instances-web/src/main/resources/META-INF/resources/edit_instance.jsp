...
<aui:form action="<%= editInstanceURL %>" cssClass="container-fluid container-fluid-max-xl" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + liferayPortletResponse.getNamespace() + "saveCompany();" %>'>
	...
	<aui:fieldset-group>
		<aui:fieldset>
			...
			<aui:input disabled="<%= (selCompany != null) && (selCompany.getCompanyId() == PortalInstancesLocalServiceUtil.getDefaultCompanyId()) %>" inlineLabel="right" labelCssClass="simple-toggle-switch" label="active" name="active_checkbox" type="toggle-switch" value="<%= (selCompany != null) ? selCompany.isActive() : true %>" />
...
<aui:script>
	function <portlet:namespace />saveCompany() {
		...
		if (form) {
			...
			var activeCheckbox = form.querySelector(
				'#<portlet:namespace />active_checkbox'
			);

			var activeHidden = form.querySelector(
				'#<portlet:namespace />active'
			);

			if (activeCheckbox) {
				if (activeHidden) {
					activeHidden.setAttribute(
						'value',
